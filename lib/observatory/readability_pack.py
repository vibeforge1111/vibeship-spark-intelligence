...
def generate_start_here(data: Dict[int, Dict[str, Any]], current_snapshot: Dict[str, Any]) -> str:
    ...
    lines.append("3. [[report_center|Report Center]] - report hub with direct links to Claude analyses.")
    lines.append("4. [[stages/08-advisory|Stage 8 - Advisory]] - advisory-specific health + source outcomes.")
    lines.append("5. [[advisory_reverse_engineering|Advisory Reverse Engineering]] - suppression logic and tuning levers.")
    lines.append("6. [[troubleshooting_by_symptom|Troubleshooting by Symptom]] - fast diagnosis by failure pattern.")
    lines.append("7. [[changes_since_last_regen|Changes Since Last Regen]] - metric deltas from previous generation.")
    lines.append("8. [[topic_finder|Topic Finder]] - jump to exact page for a specific question.")
    lines.append("9. [[glossary|Glossary]] - term definitions for moving parts.")
    lines.append("")
    ...
    lines.append("3. Review [[report_center]] for newest audits/reports and open linked Claude analyses.")
    lines.append("4. If advisory is weak, inspect [[explore/decisions/_index|Decision Ledger]], [[explore/feedback/_index|Implicit Feedback]], and [[explore/helpfulness/_index|Helpfulness Calibration]].")
    lines.append("5. If upstream quality is weak, inspect [[stages/05-meta-ralph|Meta-Ralph]] and [[stages/06-cognitive-learner|Cognitive]].")
    lines.append("6. Review [[changes_since_last_regen]] to validate whether fixes moved metrics.")
    lines.append("")
...
def generate_topic_finder() -> str:
    ...
    lines.append("| Where can I review every report and Claude critique? | [[report_center|Report Center]] | status column + analysis links |")
...
def generate_glossary() -> str:
    ...
    terms = [
        ("Advisory Engine", "The advisory runtime (`lib/advisory_engine_alpha.py`) that retrieves, gates, dedupes, synthesizes, and emits advice."),
